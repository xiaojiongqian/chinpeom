  ...
  <div class="flex flex-col h-full bg-white">
    <template v-if="poemStory">
      ...
      <div class="relative p-4 border-b h-16 flex items-center justify-center">
        <button @click="$router.back()" class="absolute left-4 text-success font-bold">
          &lt;
        </button>
        <h2 class="text-xl font-bold whitespace-nowrap overflow-hidden text-ellipsis mx-16">{{ poemStory.title }}</h2>
      </div>
      ...
      <div class="flex-1 overflow-y-auto p-4 hover-scrollbar">
        ...
        <div class="flex flex-col items-start space-y-1 mb-4">
            <button v-for="q in poemStory.story.questions.slice(0, 3)" :key="q" @click="askPresetQuestion(q)" class="bg-gray-200 text-xs hover:bg-gray-300 text-gray-700 py-2 px-3 rounded-md max-w-full text-left line-clamp-2">
              {{ q }}
            </button>
        </div>
        ...
        <div ref="chatHistoryRef" class="space-y-4">
          <div v-for="(message, index) in chatHistory" :key="index" class="flex" :class="message.role === 'user' ? 'justify-end' : 'justify-start'">
            <div class="max-w-xs md:max-w-md lg:max-w-lg px-4 py-2 rounded-lg" :class="message.role === 'user' ? 'bg-success-100 text-gray-800' : 'bg-gray-100 text-gray-800'">
          ...
          <div v-if="isLLMLoading" class="flex justify-start">
            <div class="max-w-xs md:max-w-md lg:max-w-lg px-4 py-2 rounded-lg bg-gray-100 text-gray-800">...</div>
          </div>
        </div>
      </div>
      ...
      <div class="border-t p-4">
        <div class="flex items-center">
          <input type="text" v-model="userInput" @keyup.enter="sendMessage" class="w-full border rounded-l-lg p-2 bg-white h-11" :placeholder="$t('poemDetail.askPlaceholder')" :disabled="isLLMLoading" />
          <button @click="sendMessage" class="bg-success text-white p-2 px-4 rounded-r-lg h-11 flex items-center justify-center" :disabled="isLLMLoading">
...
import { postToLLM } from '@/services/llmApi';
...
const formattedStoryContent = computed(() => {
  ...
  if (additional_info) markdown += `### ${t('poemDetail.additionalInfo')}\n\n${additional_info}\n\n`;
...
async function fetchPoemStory() {
  ...
  const lang = userStore.language === 'none' ? 'chinese' : (userStore.language || 'english').split('-')[0];
  const supportedLangs = ['chinese', 'english', 'french', 'german', 'japanese', 'spanish'];
  const finalLang = supportedLangs.includes(lang) ? lang : 'english';
...
async function sendMessage() {
  ...
  try {
    ...
    const messages = [initialPrompt, ...chatHistory.value.slice(-5)];
