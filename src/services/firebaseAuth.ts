...
export class FirebaseAuthService {
  ...
  async signInWithGoogle(): Promise<FirebaseAuthResult> {
    ...
    try {
      ...
      const code = this.isFirebaseError(error) ? error.code : undefined
      ...
      if (code === 'auth/network-request-failed') {
        ...
        throw new Error(message || 'Google登录失败')
      }
    }
  }
  ...
  async signInWithGoogleRedirect(): Promise<FirebaseAuthResult> {
    try {
      ...
      const message = error instanceof Error ? error.message : 'Google redirect登录失败'
