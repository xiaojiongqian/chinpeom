...
export async function loadPoemData(language: LanguageCode): Promise<Poem[]> {
  ...
  try {
    ...
    console.log(
      `[PoemTranslation] 数据解析完成, 类型: ${typeof data}, 是否数组: ${Array.isArray(data)}, 长度: ${Array.isArray(data) ? data.length : 'N/A'}`
    )
    ...
    if (data.length === 0) {
      ...
      if (firstPoem) {
        console.log(
          `[PoemTranslation] 第一首诗ID: ${firstPoem.id}, 标题: ${firstPoem.title || '无标题'}`
        )
...
export async function getTranslatedSentence({
  ...
  console.log(
    `[PoemTranslation] 获取诗句翻译: ID=${poemId}, 句子ID=${sentenceId}, 从${fromLang}到${toLang}`
  )
  ...
  if (!toSentence) {
    console.error(
      `[PoemTranslation] 找不到对应的翻译诗句: 诗ID=${poemId}, 句子ID=${sentenceId}, 语言=${toLang}`
    )
 ...
 * @returns 选择的句子信息
 */
export function selectRandomSentence(poem: Poem): {
  poemId: string
  sentenceId: number
  ...
  console.log(
    `[PoemTranslation] 随机选择结果: 句子ID=${selectedSentence.senid}, 内容=${selectedSentence.content}`
  )
...
export async function preparePoemWithTranslation(
  ...
  console.log(
    `[PoemTranslation] 翻译完成: 原文="${translation.original}", 译文="${translation.translated}"`
  )
