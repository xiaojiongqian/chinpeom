...
export async function loadPoemData(
  languages: LanguageType[] = ['chinese']
): Promise<PoemDataStore> {
  console.log(`[PoemData] loadPoemData called for languages: ${languages.join(', ')}`)
  ...
  try {
    ...
    if (languagesToLoad.length > 0) {
      ...
      // 等待所有加载任务完成
      console.log(`[PoemData] Awaiting Promise.all for ${languagesToLoad.length} file(s)`)
      await Promise.all(loadPromises)
      console.log(`[PoemData] Promise.all completed`)
    }
    ...
    isInitialized = true
    console.log('[PoemData] loadPoemData successfully finished, isInitialized:', isInitialized)

    return poemData
  } catch (error) {
    console.error('[PoemData] loadPoemData failed:', error)
 ...
 */
export function getTranslatedPoemById(
  id: string,
  language: Exclude<LanguageType, 'chinese'>
): TranslatedPoem | undefined {
  ensureInitialized()
...
export function getAllSentences(): string[] {
  ensureInitialized()
  return poemData.chinese.flatMap(poem => poem.sentence.map(s => s.content))
}
...
export function generateOptions(correctSentence: string, count: number): PoemOption[] {
  ...
  const similarSentences = allSentences
    .filter(
      sentence => sentence !== correctSentence && Math.abs(sentence.length - correctLength) <= 2
    )
