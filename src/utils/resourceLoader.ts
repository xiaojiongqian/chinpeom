...
export async function loadJsonFile<T>(path: string): Promise<T> {
  try {
    ...
    console.log(`[ResourceLoader] Attempting to fetch: ${fullPath}`) // 日志1

    try {
      const response = await fetch(fullPath)
      console.log(`[ResourceLoader] Fetched: ${fullPath}, Status: ${response.status}`) // 日志2

      if (!response.ok) {
        console.error(
          `[ResourceLoader] Fetch error for ${fullPath}: ${response.status} ${response.statusText}`
        ) // 日志3
        throw new Error(`请求失败: ${response.status} ${response.statusText}`)
      }

      const data = (await response.json()) as T
      console.log(`[ResourceLoader] Parsed JSON for: ${fullPath}`) // 日志4
      return data
    } catch (fetchError: any) {
      console.error(`[ResourceLoader] Catching fetchError for ${path}:`, fetchError) // 日志5
      // 在测试环境中尝试使用模拟数据
      if (isTestEnv) {
        // 提取语言键
        let languageKey = ''
        ...
        if (
          ...
          (window as any).mockPoems &&
          (window as any).mockPoems[languageKey]
        ) {
    ...
    console.error(`[ResourceLoader] Outer catch for ${path}:`, error) // 日志6
    throw error
  }
}
